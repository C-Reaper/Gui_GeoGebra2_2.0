...
Operation Operation_New(float Const1,float Const2,float (*Func)(float,float)){
	Operation op;
	op.Const1 = Const1;
	op.Const2 = Const2;
	op.Func = Func;
...
typedef BinTree MFunction;

MFunction MFunction_New(){
	MFunction Func = BinTree_New();	
	return Func;
}
...
float MFunction_Sqrt(float Left,float Right){
	return sqrtf(Right);
}
float MFunction_Sin(float Left,float Right){
	return sinf(Right);
}
float MFunction_Cos(float Left,float Right){
	return cosf(Right);
}
float MFunction_Tan(float Left,float Right){
	return tanf(Right);
}

void MFunction_Set(MFunction* Funcs,int Layer,int* Indices,int Dec,float Const1,float Const2,float (*Func)(float,float)){
	BinTree_Set(Funcs,Layer,Indices,(Operation[]){ {Const1,Const2,Func} },sizeof(Operation));
}

void MFunction_Push(MFunction* Funcs,int Layer,int* Indices,int Dec,float Const1,float Const2,float (*Func)(float,float)){
	BinTree_Add(Funcs,Layer,Indices,Dec,(Operation[]){ {Const1,Const2,Func} },sizeof(Operation));
}
...
float MFunction_CalculateBranch(BinBranch* b,float x){
	float Out = 0.0f;

	Operation* op = (Operation*)b->Memory;
    if(op){
		float arg1 = op->Const1;
		float arg2 = op->Const2;
		if(arg1==ARG_X) arg1 = x;
		if(b->First) 	arg1 = MFunction_CalculateBranch(b->First,x);
		if(arg2==ARG_X) arg2 = x;
		if(b->Second) 	arg2 = MFunction_CalculateBranch(b->Second,x);
		if(op->Func) 	Out = op->Func(arg1,arg2);
		else 			Out = arg1;
	}
...
MFunction MFunction_ToFunction(Vector Toks){ // After SYA -> in right precedence order
	...
	for(int i = 0;i<Toks.size;i++){
		...
		if(!Token_OpState(t->Type)) Vector_Push(&Solver,t);
            ...
            BinBranch* branch = BinBranch_New((Operation[]){Operation_New(arg1,arg2,(float (*)(float,float))MFunction_GetFunc(t->Type))},sizeof(Operation));
			if(First && First->Type==MFUNCTION_CALC){
				branch->First = (BinBranch*)First->str;
				branch->First->Parent = branch;
			}
			if(Second && Second->Type==MFUNCTION_CALC){
				branch->Second = (BinBranch*)Second->str;
...
int main(){
    if(Create("Geogebra",2500,1200,1,1,Setup,Update,Delete))
        Start();
    return 0;
}